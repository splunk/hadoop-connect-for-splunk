...
from splunklib.searchcommands import StreamingCommand, Option, validators
...
@Configuration()
class HDFSSearchCommand(StreamingCommand):

    path = Option(
        doc='''
        **Syntax:** **path=***<path>*
        **Description:** Path on HDFS to explore''',
        require=False)
    ...
    def _main_impl(self):
        logger.error("[---] Splunk Debug Main Implementation")
    ...
    def stream(self, records):
        logger.error("[---] Streaming metadata: {}".format(self._metadata))
        # metadata = json.loads(self._metadata.searchinfo)
        metadata = self._metadata
        
        self.argvals = metadata.searchinfo.args
        logger.error("[---] Streaming args: {}".format(self.argvals))
        ...
        logger.error("[---] Streaming session key: {}".format(self.sessionKey))
        logger.error("[---] Streaming owner: {}".format(self.owner))
        logger.error("[---] Streaming namespace: {}".format(self.namespace))
        logger.error("[---] Streaming krb5 principal: {}".format(self.krb5_principal))
        HadoopEnvManager.init(APP_NAME, 'nobody', self.sessionKey, self.krb5_principal)
        ...
        # logger.error("[---] Streaming Splunk Debug printing isp Inspect Results: {}".format(inspect.stack()[1])) 
        # results, dummyresults, self.settings = isp.getOrganizedResults()
        # self.keywords, self.argvals = isp.getKeywordsAndOptions()
        # # in Splunk pre 5.0 we don't get the info, so we just read it from it's standard location
        # infoPath = self.settings.get('infoPath', '')
        # logger.error("[---] Streaming Splunk Debug printing isp stuff inside hsc: {}".format(isp.getOrganizedResults()))
        # logger.error("[---] Streaming Splunk Debug printing isp keywords and argvals inside hsc: {}".format(isp.getKeywordsAndOptions()))
        # if len(infoPath) == 0:
        #     infoPath = os.path.join(getDispatchDir(self.settings.get('sid'), self.settings.get('sharedStorage', None)), 'info.csv')
        # self.info.readFrom(infoPath)

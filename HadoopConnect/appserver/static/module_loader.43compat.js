Splunk.ModuleLoader= $.klass({
    ...
    buildHierarchyAndTopLevelModules: function() {
        ...
        this._withEachModule(this._modules, function(childModule) {
            ...
            if (parentSelector) {
                ...
                if (!parentContainer) {
                    this.logger.error('Splunk.ModuleLoader - Module "' + childModule.container.attr('id') + '" could not find its declared parent module "' + parentSelector + '"');
                }
    ...
    queryStringToProp: function(args) {
        ...
        for (var i=0,l=lim; i<l; i++) {
            ...
            if (output.hasOwnProperty(key)) {
                if ($.isArray(output[key]) === false) {
                    output[key] = new Array(output[key]);
                }
                output[key].push(decodeURIComponent(value));
            } else {
    ...
    pushContexts: function() {
        ...
        if (!formValues) {
            ...
            var queryArgs = this.queryStringToProp(window.location.search);
        ...
        if (formValues) {
            ...
            context.set('search', new Splunk.Search()); // downstream modules expect a search to be in the context
            this._withEachModule(this._modules, function(module) {
                if (module.getToken !== undefined && typeof module.getToken === 'function'  && module.getToken()) {
        ...
        // and then we push from the autoRun points.
            ...
            // handle case where we want to absorb the earliest/latest from qstring
            if (queryArgs.earliest || queryArgs.latest) {
                var earliest = queryArgs.earliest || '0',
                    latest = queryArgs.latest || 'now';

                // iterate through the top level modules and set the new time range
                this._withEachModule(this._topLevelModules, function(module) {
                    var module_context = module.getContext(),
                        module_search = module_context.get('search');

                    module_search.setTimeRange(new Splunk.TimeRange(earliest, latest));
                    module_context.set('search', module_search);

                    if (module_search.isJobDispatched() === true) {
                        module_search.abandonJob();
                    }

                    module.baseContext = module_context;
                });
            }
        ...
        this._withEachModule(this._modules, function(module) {
            ...
            for (var key in tmpParams) {
                if (tmpParams.hasOwnProperty(key)) {
                    payload.push( module._buildParamName(key)  + '=' + encodeURIComponent(tmpParams[key]));
                }
            }
        });

        if (payload.length == 0) {
            this.logger.warn(sprintf('commitViewParams - no parameters to persist; app=%s view=%s', Splunk.util.getCurrentApp(), Splunk.util.getCurrentView()));
            return null;
        }
